...
* **드래그 가능한 좌표 추적:** 이미지를 드래그 앤 드롭하며 실시간으로 좌표를 추적
* **파일 및 Ren'Py 이미지 로드:** 이미지 파일 경로와 Ren'Py 이미지 객체 모두 지원
* **반투명 필터 (밝기/어둡기 전환):** 밝기/어둡기 필터로 현재 화면 위의 미리보기 뷰를 구분
* **선택된 이미지 강조:** 클릭/드래그 시 이미지를 강조하여 구분 (outline_shader 설정 시에만 적용)
* **다중 애셋 배치:** 여러 이미지나 UI 요소를 프리뷰 화면에 동시에 추가 가능
* **휴지통으로 드래그하여 이미지 삭제:** 이미지를 휴지통 아이콘 위로 드래그하여 프리뷰 모드에서 제거


## 🖼️ 설치 및 사용 가이드

Ren'Py의 `layout_tool`을 사용하면 이미지를 드래그하여 화면에 시각적으로 배치하거나 UI 요소를 조절할 수 있습니다. 이 도구는 대화 재생과 같은 상호 작용을 일시적으로 중지하여 레이아웃을 세밀하게 조정할 수 있도록 합니다.

### 설치

다운로드 또는 압축 해제 후 `game/` 폴더 아래 `layout-visualizer/` 폴더를 넣으세요. 이 폴더 안에 `bin.png`와 `layout_tool.rpy` 파일이 있는지 확인하세요.
...
* 이미지를 `fixed` 컨테이너에 배치하세요. `fixed` 컨테이너는 다른 레이아웃 요소의 간섭 없이 절대적인 고정 위치를 지정해줍니다.
...
데모 영상에서 나온 것과 같은 효과를 원한다면 FeniksDev의 아웃라인 셰이더 애셋을 다운로드하세요. 
이를 활성화하려면 `layout_tool.rpy` 파일에서 다음 줄을 찾아 각 줄의 시작 부분에 있는 `#`을 제거하세요:
...
나중에 `layout_tool.rpy` 파일이나 `bin.png` 이미지를 다른 위치로 이동하는 경우, `layout_tool.rpy` 내에서 이미지 경로를 업데이트해야 합니다. 파일을 열고 상단에 있는 `define bin_img` 줄을 확인하여 `bin.png`의 경로가 올바른지 확인하세요.
